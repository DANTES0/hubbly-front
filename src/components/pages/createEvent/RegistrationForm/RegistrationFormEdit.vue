...
import { dataTypeComponents } from './helpers/utilityType'
...
function cloneField(template: FieldTemplate): FormField {
  return { ...template, id: uuidv4(), options: { nameField: '', rules: [] } }
}
...
<template>
  <v-card style="height: fit-content" color="#F5F9FF" elevation="2" class="px-6 py-6">
    <v-card-text :class="{ drag: isDraggable }">
        ...
        <template #item="{ element }">
          <v-card outlined class="mb-3" color="white">
            ...
            <v-card-actions>
              ...
              <v-spacer />
              <v-btn
                @click="store.removeField(element.id)"
                icon="mdi-delete"
                size="small"
                variant="text"
                color="error"
      ...
      <v-alert v-if="!isDraggable && store.fields.length == 0" type="info" variant="tonal">
        Перетащите поля из панели справа
      </v-alert>
    </v-card-text>
    <v-card-action class="px-4">
      <v-btn v-if="store.fields.length" class="btn-common text-none">Сохранить шаблон</v-btn>
    </v-card-action>
  </v-card>
  <v-navigation-drawer
